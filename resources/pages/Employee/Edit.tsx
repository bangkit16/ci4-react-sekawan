...
function EditEmployee() {
  ...
  useEffect(() => {
    if (employeeData?.data?.data) {
      const { employee_name, employee_phone } = employeeData.data.data;
      setValue("employee_name", employee_name);
      setValue("employee_phone", employee_phone);
    }
  }, [employeeData, setValue]);

  const { mutate, isPending } = useMutation({
    mutationFn: (data: employeeSchemaType) =>
      EmployeesService.updateEmployee(id!, data as any),
    onSuccess: () => {
  ...
  if (isFetching) {
    return (
      <DashboardLayout
        ...
        breadcrumb="Dashboard · Employees · Edit Employee"
      >
