...
function EditVehicle() {
  ...
  useEffect(() => {
    ...
    fetchLocations().finally(() => {
      if (vehicleData?.data) {
        ...
        setValue("vehicle_ownership", data.vehicle_ownership);
        setValue("vehicle_number_plate", data.vehicle_number_plate);
        setValue("vehicle_year", Number(data.vehicle_year));
  ...
  }, [vehicleData, setValue,toast]);

  const { mutate, isPending } = useMutation({
    mutationFn: (data: vehicleSchemaType) =>
      VehiclesService.updateVehicle(id!, data as any),
    onSuccess: () => {
  ...
  if (isFetching) {
    return (
      <DashboardLayout
        ...
        breadcrumb="Dashboard · Vehicles · Edit Vehicle"
      >
