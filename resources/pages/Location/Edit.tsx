...
function EditLocation() {
  ...
  useEffect(() => {
    const fetchLocations = async () => {
      try {
        const response = await api.get("/api/listregion");
    ...
    fetchLocations().finally(() => {
      if (locationData?.data?.data) {
        const { location_name, address, type, region_id } =
  ...
  }, [locationData, setValue, toast]);

  const { mutate, isPending } = useMutation({
    mutationFn: (data: locationSchemaType) =>
      LocationsService.updateLocation(id!, data as any),
    onSuccess: () => {
  ...
  if (isFetching) {
    return (
      <DashboardLayout
        ...
        breadcrumb="Dashboard · Locations · Edit Location"
      >
        <div className="flex items-center justify-center h-64">
          <p className="text-slate-500">Loading location data...</p>
        </div>
      </DashboardLayout>
    );
  }
